"""
Framework di Valutazione Agent / Agent Evaluation Framework

Questo modulo fornisce utility per valutare sistemi agent.
...
class ScoreLevel(Enum):
    """Livelli di punteggio standardizzati / Standardized score levels"""
    EXCELLENT = 1.0
    GOOD = 0.8
    ACCEPTABLE = 0.6
    POOR = 0.3
...
class RubricDimension:
    """Definizione di una dimensione di valutazione / Definition of an evaluation dimension."""
    name: str
    weight: float
    description: str
    levels: Dict[str, str]  # level_name -> description

# Rubrica di default migliorata / Improved default rubric
DEFAULT_RUBRIC = {
    ...
    "source_quality": RubricDimension(
        name="source_quality",
        weight=0.10,
        description="Uses appropriate primary sources, authoritative",
        levels={
            "excellent": "Uses appropriate primary sources, authoritative",
...
class AgentEvaluator:
    ...
    def get_statistics(self) -> Dict:
        """Ottieni statistiche dalle valutazioni precedenti / Get statistics from previous evaluations."""
...
class TestSet:
    ...
    def get_by_tag(self, tag: str) -> List[Dict]:
        """Ottieni test per tag specifico / Get tests by specific tag."""
        if tag not in self.tags:
            return []
        return [self.tests[idx] for idx in self.tags[tag]]

def create_standard_tests() -> TestSet:
    """Create standard test set for context engineering."""
    test_set = TestSet("standard_context_engineering")
